...
logger = setup_logger("moa_spider", os.path.join(log_dir, "moa_spider.log"))
...
class MoaSpider:
    ...
    def fetch_list(self, pages=7):
        ...
        for k in range(1, pages + 1):
            for keyword in self.keywords:
                logger.info(f"正在获取第 {k} 页（每页 20 条，关键词：{keyword}）")
    ...
    def process_and_save(self, filename='农业农村部.json'):
        """
        保存为 JSON 文件
        """
        if not self.records:
