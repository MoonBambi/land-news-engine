...
from config.rmrb_settings import RMRB_COOKIES, RMRB_HEADERS, RMRB_KEYWORDS, RMRB_SEARCH_URL
...
class RmrbScrapySpider(scrapy.Spider):
    name = "rmrb_scrapy"
    custom_settings = {
        "DOWNLOAD_DELAY": 1.0,
        ...
        "RETRY_TIMES": 2,
        "LOG_LEVEL": "INFO",
        "LOG_FILE": os.path.join(LOG_DIR, f"rmrb_scrapy_{time.strftime('%Y%m%d_%H%M%S')}.log"),
    ...
    def parse_list(self, response, keyword: str, page_no: int):
        ...
        self.logger.info(f" 第 {page_no} 页解析完成，获取到 {len(content)} 条记录")
        for item in content:
            url = item.get("url")
