...
from config.agri_settings import AGRI_COOKIES, AGRI_HEADERS, AGRI_BASE_URL, AGRI_KEYWORDS
from config.logger import setup_logger

log_dir = os.path.join(project_root, "logs")
logger = setup_logger("agri_info_spider", os.path.join(log_dir, "agri_info_spider.log"))
...
class AgriInfoSpider:
    ...
    def fetch_list(self, pages=3):
        ...
        :param pages: 抓取页数
        """
        logger.info("正在获取列表信息，请稍等。。。。。。")
        for i in range(0, pages):
            for keyword in self.keywords:
                logger.info(f"正在获取第 {i+1} 页（每页 10 条，关键词：{keyword}）")
                time.sleep(1)
                ...
                try:
                    ...
                    logger.error(f"第 {i} 页处理失败: {e}")

            # 每处理完一页所有关键词后，再休眠一次，避免总耗时过长
            time.sleep(1)
        
        # 去重 URL
        self.data_url = list(dict.fromkeys(self.data_url))
        logger.info(f"总共收集到 {len(self.data_url)} 个不重复的 URL")

    def fetch_single_detail(self, url, idx):
        """
        抓取单个 URL 内容的任务函数
        """
        logger.info(f"正在抓取第 {idx} 个: {url}")
        content_text = ""
        try:
    ...
    def fetch_details(self):
        ...
        使用多线程并发抓取内容
        """
        logger.info(f"开始并发抓取 {len(self.data_url)} 个详情页...")
